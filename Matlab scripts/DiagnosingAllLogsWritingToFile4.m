...
% New metrics: dist start to end, quantifying vibrations
...
workingDir='/Users/gcharalambides/Desktop/cLogs';
...
for i=1:numel(fileList)
    ...
    if i>2 % first two are just dots
        ...
        % CMD
        cmdExist=sum(strcmp(fieldnames(openFile),'CMD'));
        if cmdExist
        ...
        % Radio
        radExist=sum(strcmp(fieldnames(openFile),'RAD'));
        if radExist    
        ...
        for i=2:length(WpDist)
            if (WpDist(i)>(WpDist(i-1)+10)) % kinda rough way to delineate when a new wp is reached
...
%         ntunMaxSpeed=max(ntunSpeed); % doesn't incorporate z
        ...
        % Vibrations
        numAbnormalXvibrations=sum(abs(AccX)>3);
        numAbnormalYvibrations=sum(abs(AccY)>3);
        numAbnormalZvibrations=sum(AccZ>-5)+sum(AccZ<-15);
        percentXvibAbnormal=numAbnormalXvibrations/size(AccX,1)*100;
        percentYvibAbnormal=numAbnormalYvibrations/size(AccY,1)*100;
        percentZvibAbnormal=numAbnormalZvibrations/size(AccZ,1)*100;
        avgPercentVibrations=mean([percentXvibAbnormal,percentYvibAbnormal,percentZvibAbnormal]);
        ...
        fprintf(fid,'%s, %.1f, %.2f, %.2f, %.2f, %i, %.1f, %.1f, %.2f, %.1f, %.2f, %.2f, %i, %i, %i, %i, %i, %i, %i, %i, %.2f, %.2f, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i, %i',...
             fileName, avgDistSkierToDrone, timeInAir, estimatedPossibleTimeInAir, avgPercentVibrations, round(avgThrustToHover), distStartToEnd, distanceTraveled,...
             EndingVoltage, currentDrained, maxSpeed, gpsAvgSpeed, round(percentNotUpdated), ...
             round(avgRSSI), round(percentRssiLost), round(avgRemRSSI), round(percentRemRssiLost), timesNo3dFix, avgNumSats, minNumSats, avgHdop, maxHdop, AHRS_EKF_USE, ATC_SLEW_YAW,...
